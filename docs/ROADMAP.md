...
- [ ] **Human multi-user collaboration** (late): live co-editing, CRDT/sync — Chunk 14; architecture prep in [`docs/architecture/ARCHITECTURE-multi-user-collab-prep.md`](architecture/ARCHITECTURE-multi-user-collab-prep.md)
...
- [x] **Keep `maps_*`** for Defaults Map + nested Blockly values (Chunk 8 Q9 step 2, option A). Join Lists + Maps toolbox drawers into **Lists & maps**. Sheets stay a separate drawer. See [`docs/design/DESIGN-sheets-vs-maps.md`](design/DESIGN-sheets-vs-maps.md).
