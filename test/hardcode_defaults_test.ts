...
Deno.test("hardcodeDefaultsMapKey inlines maps_get lookups from the Defaults Map value", () => {
  ...
  assert(listDefaultsMapEntries(workspace).some((entry) => entry.key === "COMPOSITION.territory"));

  const shell = workspace.newBlock("code_phrase");
  const lookup = createMapsGetBlock(workspace, "defaults", "COMPOSITION.territory");
  connectExpressionToDataValueShell(shell, lookup);

  assertEquals(hardcodeDefaultsMapKey(workspace, "COMPOSITION.territory"), 1);
  assertEquals(
    workspace.getAllBlocks(false).filter((block) =>
      block.type === "maps_get" &&
      block.getInputTargetBlock("KEY")?.getFieldValue("TEXT") === "COMPOSITION.territory"
    ).length,
    0,
  );
  const maps = namedMapsFromBlocklyState(Blockly.serialization.workspaces.save(workspace));
  assertEquals(maps.defaults?.["COMPOSITION.territory"], "SE");
