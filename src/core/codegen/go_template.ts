...
export function goTemplateHeader(model: MappingModel): string {
  return [
...
export function generateGoTemplate(
  ...
  const body = options?.blocklyState
    ? generateGoTemplateFromBlocklyJson(options.blocklyState, model)
    : generateFromSlots(model);
  return goTemplateHeader(model) + "\n" + body;
}

/** JSON snapshot walker — used when a live Blockly workspace is unavailable. */
export function generateGoTemplateFromBlocklyJson(
  blocklyState: unknown,
  model: MappingModel,
): string {
  return generateFromBlockly(blocklyState, model);
}

export function generateFromSlots(model: MappingModel): string {
  const lines: string[] = [];
...
function generateFromBlockly(blocklyState: unknown, model: MappingModel): string {
  ...
  for (const block of blocks) {
    ...
    if (node.type === "conversion_start") {
      if (node.next?.block) lines.push(...emitStatementChain(node.next.block));
      continue;
    }
    if (node.type === "defaults_block" || node.type === "maps_create_with") continue;
    if (node.type === "composition" || isRmContainerBlockType(node.type)) {
      if (model.slots.length) return generateFromSlots(model);
      lines.push(`{{- /* unsupported block: ${node.type} */ -}}`);
      continue;
    }
    lines.push(...emitStatementChain(node));
  }
  if (!lines.length && model.slots.length) return generateFromSlots(model);
  return lines.join("\n");
}

function isRmContainerBlockType(type: string): boolean {
  return type === "observation" || type === "evaluation" || type === "instruction" ||
    type === "action" || type === "admin_entry" || type === "cluster" ||
    type === "section" || type === "entry" || type === "element";
}
...
function emitBlock(block: BlockNode): string[] {
  ...
  switch (block.type) {
    ...
    case "for_each_list": {
      lines.push(...emitForEachList(block));
      break;
    }
    case "text":
    case "text_code": {
...
function emitForEachList(block: BlockNode): string[] {
  const varName = String(block.fields?.VAR ?? "item");
  const safeVar = /^[A-Za-z_][A-Za-z0-9_]*$/.test(varName) ? varName : "item";
  const list = block.inputs?.LIST?.block;
  const listExpr = list ? emitInlineValue(list) : 'index .Data ""';
  const lines: string[] = [`{{- range $${safeVar} := ${listExpr} -}}`];
  const body = block.inputs?.DO?.block;
  if (body) lines.push(...emitStatementChain(body));
  lines.push(`{{- end -}}`);
  return lines;
