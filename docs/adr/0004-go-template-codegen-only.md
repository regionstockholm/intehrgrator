...
**Considered:** giving Go template the same dual surface as Handlebars (Authored Template tab + codegen). Rejected because the existing Go mapping scripts (FLAT→TakeCare XML) are a reference, not the intEHRgrator authoring path. The Blockly canvas with `xml_element`/`controls_if`/`source_query` blocks is the editing surface; `generate(model, "go-template")` produces the equivalent Go template code. A future Blockly→Handlebars codegen adapter may follow the same pattern.
