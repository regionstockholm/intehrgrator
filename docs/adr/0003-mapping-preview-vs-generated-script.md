...
Output mode is session-only and defaults to Mapping preview after load. Generated Export and Test Run output are not persisted in the Project Bundle. The Blockly surface that feeds both paths is the Verifiable Mapping Subset (toolbox + Mapping Model IR; #35 / #37). Equivalence on VMS mappings is ADR 0009.
