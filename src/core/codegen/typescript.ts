...
function sheetHelpers(): string[] {
  return [
    ...
    "    if (!all) return matches.map((y) => cellOut(y, oi)).map((p) => p == null ? \"\" : String(p)).join(join);",
    "    const rec: Record<string, unknown> = {};",
    "    for (let x = 0; x < width; x++) {",
    "      if (meta[x]?.role !== \"output\") continue;",
    "      rec[s.headers[x] || String(x)] = matches.map((y) => cellOut(y, x)).map((p) => p == null ? \"\" : String(p)).join(join);",
    "    }",
    "    return rec;",
    "  }",
    "  return all ? rowMap(matches[0]!) : cellOut(matches[0]!, oi);",
