...
**Handlebars Template**:
User-authored **VMS-Hbs** conversion template stored in `ProjectBundle.mapping.handlebarsTemplate` (ADR 0009). **Mapping preview** Test Run renders it for free-form / Kintegrate targets. Output mode Handlebars executes that authored template against the Active Example when present, otherwise the generated slot template shown in **Generated conversion script(s)**.
