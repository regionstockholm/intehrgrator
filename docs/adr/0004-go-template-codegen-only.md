...
**Considered:** giving Go template the same dual surface as Handlebars (Authored Template tab + codegen). Rejected because the existing Go mapping scripts (FLAT→TakeCare XML) are a reference, not the intEHRgrator authoring path. The Blockly canvas with `xml_element`/`xml_text`/`xml_attribute`/`source_query` blocks is the editing surface; `text_code` (language = Go Template) nests raw Go snippets inside XML text nodes when needed. `generate(model, "go-template")` produces the equivalent Go template code. A future Blockly→Handlebars codegen adapter may follow the same pattern.
