...
Long-running, multi-step loads (Example Sets, GitHub clinical models as target or source schema) show `#task-progress-overlay`: a dimmed card with a spinner and one row per substep in **waiting / running / finished / failed**. The overlay blocks further clicks until the task settles. The controller exposes `getState().taskProgress`; `render()` paints the overlay and yields between steps so the UI can update. The footer `#status-main` still shows the current step as a short message. Nested loads (an Example Set that fetches a GitHub template) reuse the parent overlay.
