...
1. Save the issue’s Function definition JSON as `function-library/<safe-name>.intehr-function.json` (`kind` `intehrgrator-function`, `version` 1). Keep the author’s description.
2. Append one object to `catalog.json` `functions[]`: unique `id`, `name`, `title`, `description`, relative `file`, plus `locale` / `parameters` / `returns` / `decisionTables` when known.
