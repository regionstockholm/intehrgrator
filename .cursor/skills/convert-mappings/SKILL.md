...
4. **Translate target paths → slot ids.** Simplified FLAT (`templateId/path|code`) is not a `slotId`. Match Web Template / `pathLabel` / RM path. `_health_care_facility` and `composer` are `PARTY_IDENTIFIED` containers — use the existing slot when `list_slots` already has it (factory Defaults Map often scaffolds facility); otherwise `optional_rm_add` first, then `maps_create_with` keys `name`, `id`, `type`. Unconstrained coded text: keys `value`, `code_string`, `terminology_id`. Unconstrained quantity: keys `magnitude`, `units`. Identifier: keys `id`, `type` (RM field is `id`, not `value`). Done when every inventory row names a `slotId` or is marked unmappable (duplicate `slotId`, no source, or RM the envelope cannot express).
5. **Loops.** One `for_each_list` per repeating target container (`source_query_node` in LIST for source nodes). Copy `attachSlotId` from inspect (`0..*` / `1..*`). Child `EXPRESSION` is **relative** to the loop path. Composition-level fields stay absolute (`$.RekvisisjonId`) even inside a loop. Sibling `C_ARCHETYPE_ROOT` `at0000` paths use locator predicates (`…//content[openEHR-EHR-ACTION.medication.v1]`); attach still prefers `0..*` if ids collide. Done when `loops[]` and `loopVar` match inspect.
6. **Rules.** Prefer a **Decision table** (`replace_sheets` `kind: "decision-table"` + `decision_table` block) when several inputs / don't-care / FIRST|UNIQUE|COLLECT are easier to read than nested `if`. Prefer **`sheet_lookup`** for 1-key terminology. Keep `logic_ternary` for a single guard (e.g. emit diagnosis only when code and name exist). Hardcoded OPT-only codes (`codeFixed`) can stay `text` plus the fixed code. Grammatical list join (Swedish *och*, Oxford comma, similar helpers): `list_function_library` / `load_function` if [`function-library/index.md`](../../../function-library/index.md) already has it — do not invent a `join_list` builtin. Done when each rule has one construct, not both a table and nested `if` for the same slot.
7. **Emit and apply.** Write the v2 envelope. `optional_rm_add` before import only for catalog rows still listed by `list_optional_rm` (skip facility when scaffolding already inserted it). `replace_sheets` then `import_suggestions`. `set_instance_encoding` `flat-json` when the artefact was Simplified FLAT. Done when import `applied` matches the inventory (read `errors` / `skipped`) and `loopsAccepted` matches `loops[]`.
...
| `"path/_health_care_facility\|name"` | party slot `…//context/EVENT_CONTEXT/health_care_facility` (`optional_rm_add` only if `list_optional_rm` still lists it) |
| `"composer\|name"` | party slot `…//composer` |
| `{{ range $i, $row := .Items }}` … `:{{ $i }}` | `for_each_list` + `source_query_node` `$.Items`; relative fields; no `:n` index in `slotId` |
...
`pathLabel` uses the template **name constraint** when present (Vårdenhet vs Vårdgivare). Copy `slotId` from inspect; sibling ELEMENTs that share `at0003` are `items[at0003, 'Identifierare']` vs `items[at0003, 'Organisationsnummer']`.
