...
- **Handlebars** (Output mode) — executes the conversion template shown in **Generated conversion script(s)** against the Active Example: the **Authored Handlebars Template** when present, otherwise the generated slot template (`generate(model, "handlebars")`). Same generate-then-run seam as TypeScript (`renderHandlebars` + evaluated slot bag + example data).
...
- **XQuery** — generate a script; in-app execution not implemented yet.
...
**Considered:** executing generated Handlebars scripts in Test Run. Issue #100 adopted this for Output mode Handlebars: Conversion Test Run runs the same text as **Generated conversion script(s)**. Blockly→Handlebars codegen beyond the slot-comment fallback remains a later chunk.
